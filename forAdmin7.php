...
<?php
    ...
    /* คำสั่ง SQL ในส่วนใช้สำหรับ Join ตารางในฐานข้อมูล */
...
<html>
    <head>
    <meta charset="UTF-8"> <!-- กำหนดภาษาสำหรับเว็บไซต์ -->
    </head>
    ...
    <body style="background-image: url(img/bbb1.jpg); background-repeat:repeat;background-attachment:fixed;background-position: center">
        <form method="post" name="frm" class="detail" action="QueueOldupdate.php" enctype="multipart/form-data" id="form1">  
...
<center>
  <table style="font-size: 16px;width: 100%;border:3px solid black" align="center" border="2">
      ...
      <a href="forAdmin.php"><input type="button" value="กลับหน้าหลัก" name="กลับหน้าหลัก"></a>
  <br><br>
               <tr>
                <th width="100">สถานะ</th> <!-- สร้างตารางเพื่อแสดงสถานะ -->
                <th width="200">ชื่อ</th> <!-- สร้างตารางเพื่อแสดงชื่อ -->
                <th width="200">นามสกุล</th> <!-- สร้างตารางเพื่อแสดงนามสกุล -->
                <th width="300">แพทย์</th> <!-- สร้างแสดงชื่อแพทย์ -->
                <th width="200">โรงพยาบาล</th> <!-- สร้างตารางเพื่อแสดงชื่อโรงพยาบาล -->
                <th width="250">โรคประจำตัว</th> <!-- สร้างตารางเพื่อแสดงโรคประจำตัว -->
                <th width="100">วันที่และเวลานัด</th> <!-- สร้างตารางเพื่อแสดงวันและเวลาที่นัด -->
            </tr>
            <?php
                /* คำสั่ง SQL ในส่วนใช้สำหรับดึงข้อมูลจากฐานข้อมูลมาแสดงแบบ Array */
                while($objResult10 = mysqli_fetch_array($query, MYSQLI_ASSOC))
                { 	
            ?>
        <tr >
            <td  width="4%" align="center"><a href="QueueOldupdate.php?Queue_Opstatus=<?=$objResult10["Queue_Opstatus"];?>&&Queue_Op=<?=$objResult10["Queue_Op"];?>">
        ...
        ?>
         ...
         <nav>
        <ul class="pagination">
        <a href="forAdmin7.php?page=1" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
        </a>
        <?php for($i=1;$i<=$total_page;$i++){ ?>
        <a href="forAdmin7.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
        <?php } ?>
        <a href="forAdmin7.php?page=<?php echo $total_page;?>" aria-label="Next">
