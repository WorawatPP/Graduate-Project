...
<body style="background-image: url(img/bbb1.jpg); background-repeat:repeat;background-attachment:fixed;background-position: center">
    ...
    <form name="frmSearch" method="post" class="detail" action="forAdmin8.php">
    ...
    <option value="Op_Name"<?php if($_POST["ddlSelect"]=="Op_Name"){echo"selected";}?>>ชื่อ</option>

    <option value="Op_Surname" <?php if($_POST["ddlSelect"]=="Op_Surname"){echo"selected";}?>>นามสกุล</option>

    <option value="Op_Sex" <?php if($_POST["ddlSelect"]=="Op_Sex"){echo"selected";}?>>เพศ</option>

    <option value="Op_Age" <?php if($_POST["ddlSelect"]=="Op_Age"){echo"selected";}?>>อายุ</option>
        ...
        <td><div align="center"><input type="submit" value="ค้นหา" ></td>
                <td><a href="forAdmin.php"><input type="button" value="กลับหน้าหลัก"></a></td>
            </tr>
          </table>
       ...
       <?php
        ...
        $start = ($page - 1) * $perpage;
          ...
          $q = "select * from oldpeople WHERE 1  limit {$start} , {$perpage}";
                          if($dd != "" and  $txt  != "")
                       {
                         $q = "select * from oldpeople WHERE 1";
    ...
    <center>
         <nav>
        <ul class="pagination">
        <a href="forAdmin8.php?page=1" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
        </a>
        <?php for($i=1;$i<=$total_page;$i++){ ?>
        <a href="forAdmin8.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
        <?php } ?>
        <a href="forAdmin8.php?page=<?php echo $total_page;?>" aria-label="Next">
